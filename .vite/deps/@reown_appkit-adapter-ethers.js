import {
  ...
  WcConstantsUtil,
  WcHelpersUtil
} from "./chunk-PT64G3JH.js";
import "./chunk-Z7TQ6PIM.js";
import "./chunk-VOPCLWV4.js";
import "./chunk-6JE3SBOK.js";
import {
...
import "./chunk-3KI7T55T.js";
import "./chunk-BUNK4M3E.js";
...
var EthersAdapter = class extends AdapterBlueprint {
  ...
  async createEthersConfig(options) {
    ...
    async function getSafeProvider() {
      const { SafeProvider } = await import("./SafeProvider-56BCHGAQ.js");
      const { default: SafeAppsSDK } = await import("./esm-MNG4RKJZ.js");
    ...
    async function getCoinbaseProvider() {
      try {
        const { createCoinbaseWalletSDK } = await import("./dist-KMLLXYVU.js");
        if (typeof window === "undefined") {
          return void 0;
        }
  ...
  async syncConnection(params) {
    ...
    const connector = this.connectors.find((c) => c.id === id2);
    ...
    this.listenProviderEvents(id2, selectedProvider);
    if (!accounts[0]) {
  ...
  eip6963EventHandler(event) {
    if (event.detail) {
      ...
      if (!existingConnector) {
        ...
        const id2 = info?.rdns || info?.name || info?.uuid;
        if (type && this.namespace && id2) {
          this.addConnector({
            id: id2,
            type,
            imageUrl: info?.icon,
  ...
  async connect({ id: id2, address, type, chainId, socialUri }) {
    const connector = this.connectors.find((c) => HelpersUtil.isLowerCaseMatch(c.id, id2));
    if (!connector) {
      throw new Error("Connector not found");
    }
    ...
    if (type === ConstantsUtil2.CONNECTOR_TYPE_AUTH) {
      ...
      const caipNetwork = this.getCaipNetworks().find((n2) => n2.id.toString() === chainId?.toString());
      accounts = [_address];
      this.addConnection({
        connectorId: id2,
