...
const UserProfile = ({ userProfile, userAddress }) => {
  ...
  const unredeemedPoints = isProfileLoading
    ? 0
    : parseInt(profile.totalPointsEarned) -
      parseInt(profile.totalPointsRedeemed);
  const completionRate =
    isProfileLoading || parseInt(profile.articlesRead) === 0
      ? 0
      : Math.round(
          (parseInt(profile.quizzesTaken) / parseInt(profile.articlesRead)) *
            100
        );
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              <CardTitle className="text-2xl text-slate-100">
                User #{isProfileLoading ? "Loading..." : profile.userId}
              </CardTitle>
              <CardDescription className="text-base text-slate-200">
                {isProfileLoading
                  ? "Loading membership details..."
                  : "Member since joining the platform"}
              </CardDescription>
        ...
        <Card className="bg-slate-800/20 backdrop-blur-xl border border-slate-700/10 text-white">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Progress
                value={isProfileLoading ? 0 : completionRate}
                className="h-2"
              />
            </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-2 text-slate-200">
                ...
                <span>
                  {isProfileLoading
                    ? "0"
                    : 500 - (parseInt(profile.totalPointsEarned) % 500)}
                </span>
              </div>
              <Progress
                value={
                  isProfileLoading
                    ? 0
                    : (parseInt(profile.totalPointsEarned) % 500) / 5
                }
                className="h-2"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              <div className="text-center">
                <div className="text-lg font-bold text-green-400">
                  {isProfileLoading ? (
                    <Loader2 className="w-6 h-6 animate-spin" />
                  ) : parseInt(profile.articlesRead) === 0 ? (
                    "0"
                  ) : (
                    (
                      parseInt(profile.totalPointsEarned) /
                      parseInt(profile.articlesRead)
                    ).toFixed(0)
                  )}
                </div>
              ...
              <div className="text-center">
                <div className="text-lg font-bold text-emerald-400">
                  {isProfileLoading ? (
                    <Loader2 className="w-6 h-6 animate-spin" />
                  ) : parseInt(profile.quizzesTaken) === 0 ? (
                    "0"
                  ) : (
                    (
                      parseInt(profile.totalPointsEarned) /
                      parseInt(profile.quizzesTaken)
                    ).toFixed(0)
                  )}
                </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="text-center p-4 bg-purple-500/10 border border-purple-400/20 rounded-lg">
              ...
              <div className="text-xs text-slate-300">
                {isProfileLoading
                  ? "0 XFI"
                  : `≈ ${(unredeemedPoints * 0.1).toFixed(2)} XFI tokens`}
              </div>
            </div>

            <Button
              className="w-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 border-0"
              disabled={isProfileLoading || unredeemedPoints === 0}
            >
              Claim{" "}
              {isProfileLoading ? "0" : (unredeemedPoints * 0.1).toFixed(1)} XFI
            </Button>
          </CardContent>
        </Card>
      </div>
              ...
              <>
                <div className="flex items-center space-x-3 p-3 bg-green-500/10 border border-green-400/20 rounded-lg">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-slate-100">
                      Earned Badge #4 - Scholar
                    </p>
                    <p className="text-xs text-slate-300">
                      Reached 2000 points milestone
                    </p>
                  </div>
                ...
                <div className="flex items-center space-x-3 p-3 bg-emerald-500/10 border border-emerald-400/20 rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-300">
                      Blockchain Fundamentals - 300 points
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3 p-3 bg-purple-500/10 border border-purple-400/20 rounded-lg">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-slate-100">
                      Redeemed 1000 Points
                    </p>
                    <p className="text-xs text-slate-300">
                      Claimed 100 XFI tokens
                    </p>
                  </div>
                </div>
              </>
