...
const UserDashboardContent = memo(({ userProfile, unredeemedPoints, isUserRegistered, isPendingRegistration, refreshUserProfile, isLoading, handleUserRegistration, registrationError, address, isConnected, isRegistering }) => {
  const [activeTab, setActiveTab] = useState("dashboard");
  const [showRegistrationPrompt, setShowRegistrationPrompt] = useState(false); // Add missing state declaration
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <Card className="max-w-md w-full bg-slate-800 border-slate-700 shadow-2xl">
            ...
            <CardContent className="text-center space-y-4">
              <p className="text-slate-300 leading-relaxed">Start your learning journey to register your profile on the blockchain and earn XFI tokens and NFT badges!</p>
              {registrationError && (
              ...
              <div className="space-y-3">
                ...
                <p className="text-xs text-slate-400">This will register your profile by interacting with a learning module on the blockchain</p>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      {(isUserRegistered || isPendingRegistration) && (
        <>
          ...
          >
            {isLoading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : "Refresh Profile"}
          </Button>
            ...
            <div className="lg:w-80 space-y-6">
                ...
                <CardContent className="relative z-10 p-8">
                  <div className="flex items-center space-x-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400 font-mono bg-slate-700 px-2 py-1 rounded-lg">{address}</p>
                    </div>
                  </div>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-slate-700/80 rounded-xl border border-slate-600/50">
                      ...
                      <span className="text-xl font-bold text-slate-100">{userProfile ? userProfile.totalPointsEarned : 0}</span>
                    </div>
                    <div className="flex items-center justify-between p-4 bg-gradient-to-r from-slate-700/80 to-slate-600/80 rounded-xl border border-slate-500/50">
                      ...
                      <span className="text-xl font-bold bg-gradient-to-r from-emerald-400 to-purple-400 bg-clip-text text-transparent">{unredeemedPoints || 0}</span>
                    </div>
                    <Button
              ...
              <Card className="border-0 shadow-lg bg-slate-800/95 backdrop-blur-xl border border-slate-700/50 rounded-xl">
                ...
                <CardContent className="space-y-4">
                  {[
                    { icon: BookOpen, label: "Articles", value: userProfile ? userProfile.articlesRead : 0, color: "text-emerald-400", bg: "bg-slate-700", border: "border-slate-600" },
                    { icon: Target, label: "Quizzes", value: userProfile ? userProfile.quizzesTaken : 0, color: "text-purple-400", bg: "bg-slate-700", border: "border-slate-600" },
                    { icon: Award, label: "Badges", value: userProfile ? userProfile.badges : 0, color: "text-sky-400", bg: "bg-slate-700", border: "border-slate-600" },
                  ].map((stat, index) => (
                    <div key={index} className={`flex items-center justify-between p-4 ${stat.bg}/80 rounded-xl border ${stat.border}/50 hover:shadow-sm transition-all duration-300`}>
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
                <TabsList className="bg-slate-800/80 border border-slate-700 p-2 shadow-lg backdrop-blur-xl rounded-2xl">
                  {[
                    ...
                    <TabsTrigger key={tab.value} value={tab.value} className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-emerald-400 data-[state=active]:to-purple-500 data-[state=active]:text-white data-[state=active]:shadow-lg rounded-xl px-6 py-3 font-medium transition-all duration-300 text-slate-300">
                ...
                <TabsContent value="content">
                  <ContentLibrary onTabChange={handleTabChange} /> {/* Pass callback */}
                </TabsContent>
          ...
          {isPendingRegistration && (
            <Alert className="mt-6 bg-yellow-900/20 border-yellow-500/30 text-left">
              ...
              <AlertDescription className="text-yellow-300">Your profile is pending registration due to no available content. You can browse the platform, but some features will be available once content is added.</AlertDescription>
            </Alert>
          )}
        </>
      )}
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold bg-gradient-to-r from-emerald-400 via-purple-400 to-sky-400 bg-clip-text text-transparent">LearnBlock</h1>
              ...
              {(isUserRegistered || isPendingRegistration) && (
                <nav className="hidden md:flex items-center space-x-6">
                  {["Overview", "Courses", "Quizzes", "Badges"].map((item) => (
                    <button key={item} className="relative text-slate-300 hover:text-emerald-400 font-medium transition-all duration-300 group">
                      {item}
          ...
          <Card className="max-w-lg w-full bg-slate-800/95 backdrop-blur-xl border-slate-700 shadow-2xl">
            <CardContent className="p-8 text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-slate-300 leading-relaxed">Your wallet is not connected. Please connect to access the learning platform.</p>
              </div>
